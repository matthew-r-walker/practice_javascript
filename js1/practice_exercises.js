...
var profileReport = {
    ...
    getActiveCount: function () {
        ...
        profiles.forEach(function (element) {
            if(element.isActive === true) {
                activeCount.push(element);
            }
        });
        return activeCount.length;
    },
    getInactiveCount: function () {
        return profileReport.getProfileCount() - profileReport.getActiveCount();
    },
    ...
    getAverageBalance: function () {
        return parseFloat((profileReport.sumOfAllBalances() / profileReport.getProfileCount()).toFixed(2));
    },
    allBalancesLowToHigh: function () {
        var allBalances = [];
        profiles.forEach(function (element) {
            allBalances.push({name: element.name, amount: parseFloat(element.balance.replace("$","").replace(/,/g,""))});
        });
        allBalances.sort(function (a, b) {
            return a.amount - b.amount;
        });
        return allBalances;
    },
    getLowestBalance: function () {
        return profileReport.allBalancesLowToHigh()[0].name;
    },
    getHighestBalance: function () {
        return profileReport.allBalancesLowToHigh()[profileReport.allBalancesLowToHigh().length - 1].name;
    },
    getMostFavoriteFruit: function () {
        var favFruits = [];
        profiles.forEach(function (element) {
            favFruits.push(element.favoriteFruit)
        });
        function mode(arr){
            return arr.sort((a,b) =>
